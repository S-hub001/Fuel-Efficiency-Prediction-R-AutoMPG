...
# Set the file path
file_path <- "E:/Shanzay/Prob&Stats/autompg.csv"
...
#Interactive Histogram (HorsePower)
plot_ly(data_cleaned, 
        x = ~HorsePower, 
        type = "histogram",
        marker = list(
          color = 'deepskyblue3',               # fill color
          line = list(color = 'blue4',     # outline color
                      width = 1)           # outline thickness
        )) %>%
  layout(title = "HorsePower Histogram After Cleaning",
         xaxis = list(title = "HorsePower"),
         yaxis = list(title = "Count"))


#Interactive Boxplot (Acceleration)
plot_ly(data_cleaned, 
        y = ~acceleration, 
        type = "box",
  ...
  layout(title = "Acceleration Boxplot After Cleaning",
...
plot(data$origin, data$MPG, 
     main = "SLR: mpg.Y. vs Weight", 
     xlab = "origin", 
     ylab = "mpg.Y.", 
     col = "black")
...
#multicollinearity between predictors theselves
#### Displacement, Weight, and Cylinders are highly correlated with each other.
library(car)   
vif(MLM)
...
# ============================================
# Task 6: Interactive Visualization
# ============================================
...
data <- read.csv("E:/Shanzay/Prob&Stats/autompg.csv")
names(data)[names(data) == "mpg.Y."] <- "MPG"
data$WeightCategory <- cut(
  data$Weight,
...
data$text <- paste(
  "MPG:", data$MPG,
  "<br>HorsePower:", data$HorsePower,
  "<br>Weight:", data$Weight,
  "<br>Acceleration:", data$acceleration,
  "<br>Cylinders:", data$Cylinders,
  "<br>Displacement:", data$Displacement,
  "<br>Model Year:", data$ModelYear,
  "<br>Origin:", data$origin,
  "<br>Car Name:", data$CarName
)

# 2. Wrap in a SharedData object
shared <- SharedData$new(data)

# 3. Build the animated scatter, pointing to shared
fig <- plot_ly(
  data = shared,
  x = ~HorsePower,
  y = ~MPG,
  split = ~WeightCategory,
  ...
  layout(
    title = "HorsePower vs MPG by WeightCategory",
    plot_bgcolor = "black", paper_bgcolor = "black",
    font = list(color = "white"),
    xaxis = list(title="HorsePower", gridcolor="gray"),
    yaxis = list(title="MPG", gridcolor="gray"),
    legend = list(title=list(text="Weight Category")),
    updatemenus = list(
      list(
        type="buttons", showactive=FALSE,
        x=0.85, y=1.05, direction="right",
...
# 4. Create dropdown filters: Origin and Cylinders
origin_filter    <- filter_select(id = "origin",    label = "Filter by Origin",    sharedData = shared, group = ~origin)
cylinders_filter <- filter_select(id = "Cylinders", label = "Filter by Cylinders", sharedData = shared, group = ~Cylinders)


# Acceleration
acceleration_slider <- filter_slider(
  id = "acceleration", label = "Filter by Acceleration",
  sharedData = shared, column = ~acceleration
)

# Diplacement
displacement_slider <- filter_slider(
  id = "displacement",
  label = "Filter by Displacement",
  sharedData = shared,
  column = ~Displacement
)

# 5. Layout filters and plot side by side
library(htmltools)
  ...
  tags$html(
    tags$head(
      tags$style(HTML("
        ...
        .plot-container {
          flex-grow: 1;
          height: calc(100% - 60px); /* Adjust height based on filter bar */
        }
      "))
    ),
    tags$body(
...
# 6. Save as a self-contained HTML and view
save_html(widget_page, "interactive_scatterplot.html")
browseURL("interactive_scatterplot.html")
